...
/** Single unit test to be registered with the Unreal Automation Test Framework. Implementation based on IMPLEMENT_SIMPLE_AUTOMATION_TEST_PRIVATE macro. */
class FAutomationForgeTestAutomationPluginAutomationTestFrameworkUnitTest : FAutomationTestBase
{
public:
    FAutomationForgeTestAutomationPluginAutomationTestFrameworkUnitTest(
        const FString& InTestName, const FAutomationForgeUnitTestMetaData& InTestMetaData);
...
private:
    ...
    FAutomationForgeUnitTestMetaData TestMetaData;
    /** Global test map settings (suppress warnings or errors). */
	FAutomationForgeTestMapSettings UnitTestSettings;
